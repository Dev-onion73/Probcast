...
```
or (if you have a main pipeline script that detects or trains the model as needed):
...
If you have set up a Streamlit dashboard for real-time output and log/metric/trend visualization:

```bash
streamlit run streamlit_app.py
```

- The dashboard shows entity status, risk tables, interactive logs, and usage metric trends.
- Use the sidebar navigation to switch between main status and individual log/metric streams per entity or connector.
...
- See [`PROBCAST_IMPLEMENTATION_GUIDE.md`](./PROBCAST_IMPLEMENTATION_GUIDE.md) for architectural details, full schema and flows.
- For L4+ model and synthetic dataset details, see future tracks/L3-L6 implementation docs.
- For further technical or research context, see [`PROBCAST_PROJECT_DOCUMENT.md`](./PROBCAST_PROJECT_DOCUMENT.md).
